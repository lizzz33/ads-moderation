...
def test_create_user(some_user: Mapping[str, Any], app_client: TestClient):
...
@pytest.mark.parametrize("name", ["Иванов И.И."])
@pytest.mark.parametrize("password", [PASSWORD])
def test_deactivate_user(app_client: TestClient, some_user: Mapping[str, Any]):
    update_response = app_client.patch(
        f"/users/deactivate/{some_user['id']}",
        cookies={"x-user-id": str(some_user["id"])},
    )
...
@pytest.mark.parametrize("name", ["Иванов И.И."])
@pytest.mark.parametrize("password", [PASSWORD])
def test_delete_user(app_client: TestClient, some_user: Mapping[str, Any]):
    delete_response = app_client.delete(
        f"/users/{some_user['id']}", cookies={"x-user-id": str(some_user["id"])}
    )
...
@pytest.mark.parametrize("name", ["Иванов И.И."])
@pytest.mark.parametrize("password", [PASSWORD])
def test_get_many_users(app_client: TestClient, some_user: Mapping[str, Any]):
    get_users_response = app_client.get("/users")
...
@pytest.mark.parametrize("name", ["Иванов И.И."])
@pytest.mark.parametrize("password", [PASSWORD])
def test_login_user(app_client: TestClient, some_user: Mapping[str, Any]):
...
@pytest.mark.parametrize("name", ["Иванов И.И."])
@pytest.mark.parametrize("password", [PASSWORD])
def test_get_current_user(app_client: TestClient, some_user: Mapping[str, Any]):
